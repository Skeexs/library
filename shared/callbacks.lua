...
if IS_SERVER then
    ...
    _G.RegisterServerCallback = function(args)
        ...
        local eventData = RegisterNetEvent('pmc__server_callback:' .. eventName, function(packed, src, cb)
            ...
            if not source then
                ...
                -- return the data
                TriggerClientEvent(('pmc__client_callback_response:%s:%s'):format(eventName, source), source,
    ...
    _G.TriggerClientCallback = function(args)
        ensure(args, 'table'); ensure(args.source, 'string', 'number'); ensure(args.eventName, 'string'); ensure(
            args.args, 'table', 'nil'); ensure(args.timeout, 'number', 'nil'); ensure(args.timedout, 'function', 'nil'); ensure(
            args.callback, 'function', 'nil')
        ...
        if tonumber(args.source) >= 0 then
            ...
            local eventData = RegisterNetEvent(
                ('pmc__callback_retval:%s:%s:%s'):format(args.source, args.eventName, ticket), function(packed)
            ...
            TriggerClientEvent(('pmc__client_callback:%s'):format(args.eventName), args.source,
...
if not IS_SERVER then
    ...
    _G.RegisterClientCallback = function(args)
        ...
        local eventData = RegisterNetEvent('pmc__client_callback:' .. eventName, function(packed, ticket)
            ...
            if type(ticket) == 'function' then
                ...
                TriggerServerEvent(('pmc__callback_retval:%s:%s:%s'):format(SERVER_ID, eventName, ticket),
    ...
    -- [@function callback - Asynchronous response]
    _G.TriggerServerCallback = function(args)
        ensure(args, 'table'); ensure(args.args, 'table', 'nil'); ensure(args.eventName, 'string'); ensure(args.timeout,
            'number', 'nil'); ensure(args.timedout, 'function', 'nil'); ensure(args.callback, 'function', 'nil')
    ...
    -- [@function callback - Asynchronous response]
    _G.TriggerClientCallback = function(args)
        ensure(args, 'table'); ensure(args.eventName, 'string'); ensure(args.args, 'table', 'nil'); ensure(args.timeout,
            'number', 'nil'); ensure(args.timedout, 'function', 'nil'); ensure(args.callback, 'function', 'nil')
